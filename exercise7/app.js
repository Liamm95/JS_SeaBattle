...
class App {
  ...
  shipArrangement(player, shipCount, maxShipLength) {
    for (let i = 1; i <= shipCount; i++) {
      ...
      if (shipLength > maxShipLength) {
        alert(
          `Ошибка! Длина корабля (${shipLength}) превышает максимальную допустимую (${maxShipLength})`,
        );
        i--;
        continue;
      }
  ...
  run() {
    const typeOpponent = parseInt(
      prompt("Введите тип оппонента: 1 — человек, 0 — компьютер"),
    );
    ...
    } else if (typeOpponent === 0) {
      const bot = new AIPlayer(this.boardSize);
      this.setSecondPlayer = bot;
      this.shipArrangement(
        this.secondPlayer,
        this.getMaxShips,
        this.getMaxLengthShip,
